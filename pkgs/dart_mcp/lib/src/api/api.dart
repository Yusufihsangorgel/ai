...
///   or hyphens (`-`). Implementations SHOULD use reverse DNS notation (e.g.,
///   `com.example/` rather than `example.com/`), and any prefix whose second
///   label is `modelcontextprotocol` or `mcp` is reserved for MCP use. For
///   example: `io.modelcontextprotocol/`, `dev.mcp/`,
///   `org.modelcontextprotocol.api/`, and `com.mcp.tools/` are all reserved,
///   while `com.example.mcp/` is not, because its second label is `example`.
///   Protocol versions before 2025-11-25 reserved any prefix carrying
///   `modelcontextprotocol` or `mcp` in a label other than the last.
...
/// rule. Their values MUST follow the W3C Trace Context and W3C Baggage
