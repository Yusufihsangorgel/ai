...
abstract base class MCPServer extends MCPBase {
  ...
  FutureOr<ServerCapabilities> initialize(
    ...
    // Answering `server/discover` is how a server says it speaks the
    // request-scoped protocol, so only a server serving that revision
    // registers it. A client probing a legacy server gets the method not found
    // error it needs to fall back to the `initialize` handshake, which is what
    // the stdio backward compatibility rules ask of us.
  ...
  /// server serves and the capabilities [initialize] registered.
  ///
  ...
  ///
  /// The identity, result type, and caching hints the schema also requires are
  /// added by the request-scoped dispatcher, which stamps them on every result
  /// it returns, so this only answers the fields which are specific to
