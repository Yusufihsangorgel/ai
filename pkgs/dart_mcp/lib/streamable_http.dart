...
///
/// A client posts with `streamableHttpClientChannel` and can decode SSE
/// responses with `sseMessageStream`.
library;
...
/// Creates a [StreamChannel] which sends each JSON-RPC message to [uri] in a
/// separate Streamable HTTP POST.
///
/// [protocolVersion] is written to both the request metadata and the
/// `MCP-Protocol-Version` header. [clientCapabilities] and [clientInfo] are
/// added to the request metadata without removing existing parameters or
/// metadata.
///
/// Responses may be `application/json` bodies or `text/event-stream` streams.
/// A failed POST produces a JSON-RPC error for that request without closing the
/// channel. A notification acknowledged with `202 Accepted` produces no
/// inbound message.
///
/// Valid `x-mcp-header` annotations learned from `tools/list` responses are
/// mirrored on later `tools/call` requests as `Mcp-Param-{Name}` headers.
/// Tools with an invalid annotation are omitted from the list response.
///
/// The caller supplies the request-scoped protocol context. This low-level
/// channel implements only the 2026-07-28 transport. It does not send an
/// `initialize` request, negotiate a version, or fall back to an earlier
/// transport.
...
Stream<Map<String, Object?>> _sendStreamableHttpMessage(
  ...
  try {
    ...
    if (object.kind == JsonRpc2Kind.request) {
      ...
      for (final MapEntry(:key, :value) in state.headersFor(message).entries) {
...
final class _StreamableHttpClientState {
  ...
  Map<String, String> headersFor(Map<String, Object?> message) {
    ...
    if (params is! Map<String, Object?>) return const {};
    final name = params[Keys.name];
    final arguments = params[Keys.arguments];
    if (name is! String || arguments is! Map<String, Object?>) return const {};
