...
void main() {
  group('elicitation', () {
    ...
    test('a multi-select enum is a schema a client can be asked for', () {
      // Both shapes the spec lists, see
    ...
    test('an array that is not a multi-select enum is refused', () {
      for (final property in [
        ...
        // A titled entry carries a const, so a title on its own is not one.
        Schema.fromMap({
          'type': 'array',
          'items': {
            'anyOf': [
        ...
        // And it carries a title, so a const on its own is not one either.
        Schema.fromMap({
          'type': 'array',
          'items': {
            'anyOf': [
