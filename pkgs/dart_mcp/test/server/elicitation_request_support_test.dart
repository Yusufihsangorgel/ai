...
final class _ElicitingServer extends MCPServer
  ...
  _ElicitingServer(super.channel)
    ...
    registerTool(Tool(name: 'test/no-mode', inputSchema: ObjectSchema()), (
      ...
      // The revision lets a server leave `mode` out of a form request, and no
      // constructor here builds one without it.
      await elicit(
        <String, Object?>{
...
void main() {
  ...
  test('a request with no mode goes through the form check', () async {
    final result = await _call(
      'test/no-mode',
      ClientCapabilities(elicitation: ElicitationCapability(url: {})),
    );
  ...
  test('an unrecognized mode clears a plain elicitation client', () async {
    final result = await _call(
      'test/unknown',
      ClientCapabilities(elicitation: ElicitationCapability()),
    );
  ...
  test('an unrecognized mode falls back to the plain capability', () async {
    final result = await _call('test/unknown', ClientCapabilities());
