...
void main() {
    ...
    test(
      ...
      () async {
        ...
        expect(
          ...
          reason:
              'answering discovery is how a server says it speaks the '
              'request-scoped protocol, so a server on an earlier revision '
              'must not answer it',
        );
      },
    );
  ...
  group('legacy lifecycle', () {
    ...
    test('does not answer a discovery probe', () async {
      ...
      // A client which speaks both eras probes with `server/discover` before
      // anything else, and reads any error as "this server is legacy, fall
