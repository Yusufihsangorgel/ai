...
void main() {
  group('ServerConnection.discover against a real MCPServer', () {
    test('a real server\'s DiscoverResult round-trips and selects the '
      ...
      // serialization rather than a map literal a test wrote by hand.
    ...
    test('discover leaves a connection that never negotiated a legacy '
      ...
      // (It throws a LateInitializationError, an Error rather than a
      // StateError.)
...
/// A real [MCPServer] that additionally answers `server/discover`, the way
/// #596 wires it up on [MCPServer] itself. This test predates that PR
/// landing on this branch, so the handler is registered here instead of
/// inherited.
