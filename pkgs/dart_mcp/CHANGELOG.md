...
- Add `streamableHttpClientChannel`, posting each client message as a
  ...
  JSON decoded with `jsonDecode` and SSE with `sseMessageStream`. A failed
  POST, a response stream that ends without answering, and a notification
  answered with a body are all errors, for the request id or (for a
  notification) on the channel. A `tools/call` whose params or arguments are
  not a string-keyed map is a request error. Valid `x-mcp-header` annotations
  from `tools/list` are mirrored on later `tools/call` requests, including an
  integer written as a decimal; a value the tool cannot carry fails the request
  rather than going out without its header. Invalid tool definitions are
  dropped, and a `tools/list` page with no cursor replaces what earlier pages
  taught. The helper speaks only 2026-07-28 and does not negotiate a version.
